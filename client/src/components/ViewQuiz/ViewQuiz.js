    ...
    render() {
        return !this.state.isLoading ? (
            <div className="view-quiz">
                {!this.state.isAuthenticated ? <div className="not-auth">Du skal være logget ind</div> : 
                        ...
                        <div className="left">
                            ...
                            <div className="comments">
                                ...
                                <div className="input-field">
                                    <input value={this.state.inputVal} onChange={e => this.setState({inputVal: e.target.value})} type="text" placeholder="Tilføj en ny kommentar" />
                                    <button onClick={this.addComment}>Send</button>
                                </div>
                            </div>
                        </div>
                        <div className="right">
                            <div className="questions-num">{this.state.quiz.questions.length} Spørgsmål</div>
                            <div className={this.state.quiz.createdBy === localStorage.getItem('_ID') ? 'questions-wrapper' : 'questions-wrapper no-scroll'}>
                                ...
                                {this.state.quiz.createdBy !== localStorage.getItem('_ID') ? <div className="hidden"><div>Du skal have lavet denne quiz før du kan se spørgsmålene</div></div> : ''}
                            </div>
                        </div>
                    </div>
